...
unsigned long red, ir; // Global variables to store the last read values
                        ...
                        int adcRange, struct max30102_record *record, i2c_dev_t *dev)
{
    ...
    if (sampleAverage == 1)
        ret |= max30102_setFIFOAvarage(MAX30102_SAMPLEAVG_1, dev); // No averaging per FIFO record
    else if (sampleAverage == 2)
        ret |= max30102_setFIFOAvarage(MAX30102_SAMPLEAVG_2, dev);
    else if (sampleAverage == 4)
        ret |= max30102_setFIFOAvarage(MAX30102_SAMPLEAVG_4, dev);
    else if (sampleAverage == 8)
        ret |= max30102_setFIFOAvarage(MAX30102_SAMPLEAVG_8, dev);
    else if (sampleAverage == 16)
        ret |= max30102_setFIFOAvarage(MAX30102_SAMPLEAVG_16, dev);
    else if (sampleAverage == 32)
        ret |= max30102_setFIFOAvarage(MAX30102_SAMPLEAVG_32, dev);
    else
        ret |= max30102_setFIFOAvarage(MAX30102_SAMPLEAVG_4, dev);
    ...
    //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    if (ledMode == 3)
        ret |= max30102_setLEDMode(MAX30102_MODE_MULTILED, dev); // Watch all three LED channels
    else if (ledMode == 2)
        ret |= max30102_setLEDMode(MAX30102_MODE_REDIRONLY, dev); // Red and IR
    else
        ret |= max30102_setLEDMode(MAX30102_MODE_REDONLY, dev); // Red only
    record->activeLEDs = ledMode;                               // Used to control how many bytes to read from FIFO buffer

    // Particle Sensing Configuration
    //-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    if (adcRange < 4096)
        ret |= max30102_setADCRange(MAX30102_ADCRANGE_2048, dev); // 7.81pA per LSB
    else if (adcRange < 8192)
        ret |= max30102_setADCRange(MAX30102_ADCRANGE_4096, dev); // 15.63pA per LSB
    else if (adcRange < 16384)
        ret |= max30102_setADCRange(MAX30102_ADCRANGE_8192, dev); // 31.25pA per LSB
    else if (adcRange == 16384)
        ret |= max30102_setADCRange(MAX30102_ADCRANGE_16384, dev); // 62.5pA per LSB
    else
        ret |= max30102_setADCRange(MAX30102_ADCRANGE_2048, dev);

    if (sampleRate < 100)
        ret |= max30102_setSampleRate(MAX30102_SAMPLERATE_50, dev); // Take 50 samples per second
    else if (sampleRate < 200)
        ret |= max30102_setSampleRate(MAX30102_SAMPLERATE_100, dev);
    else if (sampleRate < 400)
        ret |= max30102_setSampleRate(MAX30102_SAMPLERATE_200, dev);
    else if (sampleRate < 800)
        ret |= max30102_setSampleRate(MAX30102_SAMPLERATE_400, dev);
    else if (sampleRate < 1000)
        ret |= max30102_setSampleRate(MAX30102_SAMPLERATE_800, dev);
    else if (sampleRate < 1600)
        ret |= max30102_setSampleRate(MAX30102_SAMPLERATE_1000, dev);
    else if (sampleRate < 3200)
        ret |= max30102_setSampleRate(MAX30102_SAMPLERATE_1600, dev);
    else if (sampleRate == 3200)
        ret |= max30102_setSampleRate(MAX30102_SAMPLERATE_3200, dev);
    else
        ret |= max30102_setSampleRate(MAX30102_SAMPLERATE_50, dev);

    // The longer the pulse width the longer range of detection you'll have
    // At 69us and 0.4mA it's about 2 inches
    // At 411us and 0.4mA it's about 6 inches
    if (pulseWidth < 118)
        ret |= max30102_setPulseWidth(MAX30102_PULSEWIDTH_69, dev); // Page 26, Gets us 15 bit resolution
    else if (pulseWidth < 215)
        ret |= max30102_setPulseWidth(MAX30102_PULSEWIDTH_118, dev); // 16 bit resolution
    else if (pulseWidth < 411)
        ret |= max30102_setPulseWidth(MAX30102_PULSEWIDTH_215, dev); // 17 bit resolution
    else if (pulseWidth == 411)
        ret |= max30102_setPulseWidth(MAX30102_PULSEWIDTH_411, dev); // 18 bit resolution
    else
        ret |= max30102_setPulseWidth(MAX30102_PULSEWIDTH_69, dev);
    ...
    if (ledMode > 1)
        ret |= max30102_enableSlot(2, SLOT_IR_LED, dev);
...
void max30102_test()
{
    ...
    ESP_ERROR_CHECK(max30102_initDesc(&dev, 0, I2C_MASTER_SDA_IO, I2C_MASTER_SCL_IO));
    // i2c_max30102_init();
    if (max30102_readPartID(&dev) == ESP_OK)
        ESP_LOGI(__func__, "Found MAX30102!");
    else
        ESP_LOGE(__func__, "MAX30102 not found");

    uint8_t powerLed = 0x2F;
    ...
    ESP_ERROR_CHECK(max30102_init(powerLed, sampleAverage, ledMode, sampleRate,
                                  pulseWidth, adcRange, &record, &dev));
    ...
    while (1)
    {
        ...
        vTaskDelay(pdMS_TO_TICKS(1)); // Delay to simulate processing time
        if (FLAG_INMP441)
        {
            FLAG_MAX30102 = false;
            break;
...
//     xTaskCreate(max30102_test, "max30102_test", 1024 * 5, NULL, 6, NULL);
// }
